...
void main()
{
    vec2 point=2.0*v_texCoords-vec2(1.0);
    float outlineDist=sdRoundBox(point,vec2(1.0),2.0*u_radius);
    vec4 col;
    if(outlineDist>0.0){
        col=vec4(0.0);
    }else{
        vec2 fillDim=vec2(1.0)-2.0*vec2(u_outline);
        ...
        if(u_fillType==0){
            ...
            float distance =dist(u_radialPosition,v_texCoords)/u_gradientRadius;
            if(distance>1.0){
                distance=1.0;
            }
